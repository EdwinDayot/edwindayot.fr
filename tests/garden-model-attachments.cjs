...
for (const type of ["pilea", "monstera", "calathea"]) {
  test(`${type}: petioles and blades stay connected during growth and sway`, () => {
    ...
    const stem = plant.userData.structure.children.find(
      (child) => child.isMesh,
    );
    for (const [index, leaf] of plant.userData.leaves.entries()) {
      ...
      if (stem) {
        const node = stem.geometry.parameters.path.getPoint(
          0.18 + (index / (plant.userData.leaves.length - 1)) * 0.82,
        );
        assert.ok(
          node.distanceTo(curve.getPoint(0)) < 1e-7,
          "Petiole starts on the central stem",
        );
      } else {
        assert.equal(
          curve.getPoint(0).y,
          0,
          "Rosette stems start at soil level",
        );
      }
      for (const growth of [0.1, 0.4, 1])
        for (const sway of [-0.028, 0.028, 0.178]) {
          ...
          assert.ok(
            joint.distanceTo(tip) < 1e-7,
            "Animation must not move the attachment",
          );
          const outward = new THREE.Vector3(0, 1, 0).applyEuler(
            leaf.blade.rotation,
          );
          const radial = new THREE.Vector3(
            leaf.blade.position.x,
            0,
            leaf.blade.position.z,
          ).normalize();
          assert.ok(
            outward.dot(radial) > 0,
            "Blade opens away from the centre",
          );
          const upper = new THREE.Vector3(0, 0, -1).applyEuler(
            leaf.blade.rotation,
          );
      ...
      if (type === "pilea")
        assert.ok(
          Math.abs(bounds.min.y + bounds.max.y) < 1e-6,
          "Disc centred on its petiole",
        );
      else
        assert.ok(
          Math.abs(bounds.min.y) < 1e-6,
          "Blade base begins at its petiole",
        );
    }
  });
}
