...
Epic C2.5v-b (l'écran d'enseignement et la trajectoire enseignée dans le monde, `render-campaign-teaching.js`) a eu besoin d'un survol de portée supplémentaire : la trajectoire réelle qu'un joueur s'apprête à enseigner à une Rainelle, avant confirmation. Plutôt qu'ajouter une nouvelle entrée à `TRANSPARENT_ALLOWLIST`, ce survol réutilise **exactement** la teinte et le plafond d'opacité déjà documentés pour le halo de vision (`6d9365`, opacité ≤ 0,15) — même famille visuelle, jamais une nuance inventée pour l'occasion. Aucune entrée nouvelle n'était donc nécessaire ; `tests/garden-material-audit.cjs` l'exerce quand même via une scène étendue (un brouillon d'enseignement poussé dans l'état réel, `v.sync()` réel) pour que toute dérive future (une teinte ou une opacité qui changerait) soit détectée comme n'importe quel autre matériau.
