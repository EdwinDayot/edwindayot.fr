...
Les epics de règles (schéma, simulation) se prouvent par `npm test`. Les epics de **narration** (Scénariste) et de **direction artistique** (Artisan rendu) n'ont pas d'oracle aussi net, et aucun humain ne regardera chaque résultat au moment où il est produit. Quatre mesures compensent, sans prétendre égaler un vrai regard humain :

- **Un style de référence extrait de l'existant, pas inventé par epic.** [direction-artistique.md](direction-artistique.md) documente la palette, les matériaux et les conventions de silhouette réellement en usage dans le jardin déjà livré (valeurs extraites du code, pas de teintes réinventées). Un epic d'Artisan rendu doit s'y référer explicitement plutôt que choisir librement — la cohérence vient de la contrainte du système, pas du goût individuel de chaque déclenchement.
- **Relecture multimodale réelle, pas seulement textuelle.** Pour tout epic touchant au rendu, le déclenchement qui le clôt doit produire une capture Playwright (comme le fait déjà la suite existante, voir `garden.md`) puis **lire ce fichier image avec l'outil Read** (qui gère les images) et comparer explicitement ce qu'il voit à `direction-artistique.md` — teinte hors palette, silhouette confuse, pénétration de maillage, proportion incohérente avec les espèces déjà livrées. Une relecture qui ne regarde que le code source de ce qu'elle vient d'écrire n'a rien vérifié. Le résultat de cette relecture (conforme, ou ce qui a été corrigé) est consigné dans l'entrée de `docs/campagne.md`.
- **Relecture adverse pour la narration.** Tout epic de Scénariste est relu, dans le même déclenchement, contre des critères concrets déjà écrits dans le design (grammaire narrative des sections 9 à 11, tableau des trois leviers d'intensification section 11) — jamais une approbation de complaisance de l'auteur du texte.
- **Auto-parcours scénarisés aux portes de phase.** Les scénarios des « premières 90 minutes » (section 13) et les scénarios de validation (section 16) doivent devenir des epics de test à part entière dans le backlog à chaque porte de phase — un parcours Playwright qui rejoue littéralement l'ouverture du jeu et vérifie les preuves listées par le design.

Cette limite doit rester énoncée honnêtement plutôt que maquillée : même avec une relecture multimodale, un jugement esthétique réel reste plus fiable venant d'un humain. C'est pourquoi une **galerie de suivi** existe en complément (voir plus bas) : elle ne bloque rien, mais donne à l'utilisateur un moyen réel de regarder et de corriger le guide de direction artistique si un résultat le déçoit — la relecture automatique attrape les incohérences grossières, pas le mauvais goût cohérent.
