(function (root) {
  ...
  class SaveStore {
    ...
    load(now = Date.now()) {
      ...
      if (!game && !current) {
        const old = this.read(LEGACY);
        if (old) {
          try {
            const data = R.migrate(JSON.parse(old), now);
            if (this.write(LEGACY + "-backup", old)) {
      ...
      this.save(game, now);
      return { game, summary, message: this.message };
    }
