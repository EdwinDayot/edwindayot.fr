/* Water simulation: tick and status, attached to the shared irrigation scope. */
(function (root) {
  ...
  function tick(s) {
    ...
    for (const pump of g.pumps) {
      if (pump.x < 2.5) continue;
      let budget = D.recipes.pump.flow;
      const paths = g.pumpPaths.get(pump.id),
      ...
      pump.running = budget < D.recipes.pump.flow;
    }
    const budget = new Map(g.tanks.map((t) => [t.id, D.recipes.tank.flow]));
    const pots = [...g.entities.values()]
        .filter((e) => isPot(e) && e.plant && e.plant.moisture < 65)
        .sort((a, b) => a.id.localeCompare(b.id)),
      offset = pots.length ? s.irrigationCursor % pots.length : 0;
    for (let i = 0; i < pots.length; i++) {
      const pot = pots[(i + offset) % pots.length];
      let wanted = Math.min(0.5, 65 - pot.plant.moisture);
    ...
    for (const e of g.entities.values()) {
      ...
      e.lastFlow = flow;
      e.flowing = flow > EPS;
      if (e.type !== "pump") e.running = e.flowing;
    }
    telemetry.set(s, {
  ...
  function status(s, entity) {
    ...
    if (entity.type === "pump") {
      const paths = g.pumpPaths.get(entity.id),
      ...
      return {
        ...
        message:
          flow > EPS
            ? `Remplissage des citernes · ${flow.toFixed(1)} eau/s`
            : !tanks.length
              ? "Relie la pompe à une citerne via des tuyaux"
              : "Citernes pleines · pompe en veille",
        water,
        capacity: tanks.length * 160,
        pots: 0,
        flow,
      };
    }
    const relevant = new Map(sourceTanks.map((t) => [t.id, t]));
    for (const paths of g.pumpPaths.values())
      for (const tank of g.tanks) {
    ...
    let kind = "idle",
      message = "Terreau humide · arrosage en veille";
    if (!(g.adj.get(entity.id) || []).length) {
      ...
      message =
        entity.type === "tank" && outgoing > EPS && incoming > EPS
          ? `Entrée ${incoming.toFixed(1)} · sortie ${outgoing.toFixed(1)} eau/s`
          : watering
            ? `Arrosage · ${pots.size} pot${pots.size > 1 ? "s" : ""} · ${flow.toFixed(1)} eau/s`
            : `Remplissage · ${flow.toFixed(1)} eau/s`;
    } else if (!tanks.length) {
      kind = "no-tank";
      message = "Aucun chemin d’eau depuis une citerne";
    } else if (!water) {
      kind = "empty";
      message = "Citerne vide · apporte de l’eau du ponton";
    } else if (!pots.size) {
      kind = "no-pot";
