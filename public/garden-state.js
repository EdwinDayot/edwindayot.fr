...
(function (root) {
  ...
  class GardenState {
    ...
    command(c, ctx = {}) {
      const s = this.s,
        e = s.entities.find((e) => e.id === c.id),
        fail = (message) => ({ ok: false, message });
      let message = "C’est fait.";
      const physical = [
        "plant",
        "water",
        "collect",
      ...
      if (c.type === "equip") {
        ...
        message = `Emplacement ${c.slot + 1} équipé.`;
        ...
        message = `${sp.name} planté. Un arrosage pour commencer.`;
        ...
        message = "Une pluie douce sur le terreau.";
        ...
        e.plant.ready = 0;
        message = `Récolte de ${p.name} rangée dans l’inventaire.`;
      } else if (c.type === "mine") {
        ...
        message = `Découverte : ${D.species.find((p) => p.id === cache.species).name}. Deux graines et une page au carnet.`;
        ...
        this.pay(z.cost);
        P.unlock(s, z.id);
        message = `${z.name} est accessible. Explore les caches botaniques.`;
        ...
        this.pay(cost);
        this.add(c.item, 1);
        message = `${recipe.name} ajouté à la réserve. Choisis son emplacement.`;
        ...
        message = "Objet rangé intact. Sa plante et son travail sont en pause.";
        ...
        message = "Raccordements mis à jour.";
        ...
        s.water -= q;
        e.water += q;
        message = `${q} unités versées dans la citerne.`;
        ...
        e.buffer = {};
        message = "Production du collecteur rangée dans l’inventaire.";
        ...
        e.job = null;
        s.stats.collected++;
        message = "Un jeune plant rejoint le sac. L’établi est libre.";
        ...
        message = "Iris t’offre un banc et 8 feuilles pour ta collection.";
      ...
      if (
        ...
        I.invalidate(s);
      const result = { ok: true, kind: c.type, message, id: e?.id };
  ...
  function migrate(old, now = Date.now()) {
    ...
    const g = new GardenState(null, now),
      s = g.s,
