...
(() => {
  ...
  if (heroImage && heroCaption && reelButtons.length) {
    ...
    reelButtons.forEach((button) =>
      button.addEventListener("click", () => switchReel(button)),
    );
  }

  if ("IntersectionObserver" in window && projectLinks.length) {
    const initialProject =
      projectLinks.find((link) => link.hash === window.location.hash) ||
      projectLinks[0];
  ...
  lightbox.innerHTML =
    '<button class="lightbox-close" type="button" aria-label="Fermer l\'aperçu">×</button><figure><img alt=""><figcaption></figcaption></figure>';
